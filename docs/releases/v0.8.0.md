...
- `GOV-0345-v0-8-0-phase-release-closeout-record`：补齐 `v0.8.0` final Phase / release closeout artifact，对应 Issue `#345`
  - 状态：阶段 A 建立 closeout carrier，记录 Phase `#293` 已关闭为 `closed completed`，阶段 A 前基线为 `594231b9f18a459bc64b771c486b73808ecaf764`，并补齐 `#327` post-merge closeout 记录粒度；阶段 A 合入后创建 `v0.8.0` tag / GitHub Release，阶段 B 回写 published truth。
