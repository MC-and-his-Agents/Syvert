...
- `GOV-0345-v0-8-0-phase-release-closeout-record`：补齐 `v0.8.0` final Phase / release closeout artifact，对应 Issue `#345`
  - 状态：阶段 A 建立 closeout carrier，记录 Phase `#293` 已关闭为 `closed completed`，最终主干为 `594231b9f18a459bc64b771c486b73808ecaf764`，并在 `GOV-0345` evidence 中补齐 `#327` post-merge closeout 记录粒度；阶段 A 合入后创建 `v0.8.0` tag / GitHub Release，阶段 B 回写 published truth。
...
- 阶段 A 前基线：`main == origin/main == 594231b9f18a459bc64b771c486b73808ecaf764`。
- `#312/#322/#327` closeout worktree 已清理，分支已退役。
- `v0.8.0` tag / GitHub Release 由 `#345` 阶段 A carrier 合入后创建；阶段 B 回写 published truth、发布锚点与阶段 A merge commit。
