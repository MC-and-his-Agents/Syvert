...
`v1.x` 是能力扩展阶段，不是从 `v1.1.0` 数到 `v1.9.0` 后自动跳到 `v2.0.0` 的倒计时。
...
因此，`v1.x` 可以自然出现 `v1.8.0`、`v1.9.0`、`v1.10.0` 或更长的 minor 序列。只有当上述 gate 全部满足，且需要对外声明“扩展 runtime capability contract 已稳定”时，才进入 `v2.0.0`。
