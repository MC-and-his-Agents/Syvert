...
- 契约需求：
  ...
  - 成功结果 envelope 最小字段固定为：`task_id`、`adapter_key`、`capability`、`status`、`raw`、`normalized`。
  - `normalized` 最小公共字段固定为：`platform`、`content_id`、`content_type`、`canonical_url`、`title`、`body_text`、`published_at`、`author`、`stats`、`media`。
  - `author` 最小字段固定为：`author_id`、`display_name`、`avatar_url`。
  ...
  - 对于失败任务，Core 只承载统一失败 envelope，不解释平台内部错误语义；平台特定错误细节由 adapter 通过结构化错误附带。
- 非功能需求：
