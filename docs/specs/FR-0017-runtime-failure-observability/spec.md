...
- 背景：`FR-0005` 已冻结统一错误模型，`FR-0008` 已冻结 `TaskRecord` 持久化，`FR-0011` 已冻结 task-bound 资源追踪，后续 `v0.6.0` 运行时还会引入 timeout / retry / concurrency 结果。当前缺少一层 formal contract 来回答“失败发生后，Core 至少应留下哪些可关联、可审查、可计数的运行时信号”。如果这层语义散落在 adapter 私有日志、一次性 stderr 或实现注释里，失败排查会重新退回平台私有事实，无法围绕同一 `task_id`、执行 envelope、TaskRecord 与资源追踪建立最小真相链。
- 目标：为 `v0.6.0` 冻结运行时失败可观测性的最小 contract，在不重写 `FR-0005` 错误分类的前提下，明确失败分类投影、结构化日志、最小执行指标，以及这些信号如何关联 `task_id`、TaskRecord、执行 envelope、资源追踪和 `FR-0016` 的 timeout / retry / concurrency 运行时结果。
...
- 本次纳入：
  ...
  - 冻结信号如何关联 `task_id`、TaskRecord、执行 envelope、资源追踪事件、以及 `FR-0016` timeout / retry / concurrency 运行时结果
...
- 功能需求：
  ...
  - `RuntimeStructuredLogEvent` 是 task-bound 结构化日志的最小事件 carrier；它必须能表达任务生命周期、运行时失败、retry 尝试、timeout 触发、concurrency 拒绝或收口等关键运行时节点。
  - `RuntimeExecutionMetricSample` 是最小执行指标 carrier；它只冻结可本地计数与聚合的执行事实，不要求持久化指标后端或实时采集系统。
  - 所有失败相关信号必须至少能回溯到同一 `task_id`，并在存在相关事实时回溯到 TaskRecord、failed envelope、resource trace、runtime attempt 与 concurrency slot / timeout / retry 结果。
...
- 契约需求：
  ...
  - `envelope_ref` 必须能定位同一次执行产生的 failed envelope；若 failed envelope 已作为 TaskRecord 终态结果持久化，该引用必须能与 `task_record_ref` 共同证明二者属于同一 `task_id`。
  ...
  - `RuntimeStructuredLogEvent` 至少必须包含：
    ...
    - `resource_trace_refs`
    - `runtime_result_refs`
  - `event_type` 在 `v0.6.0` 的最小集合固定为：`task_accepted`、`task_running`、`attempt_started`、`attempt_finished`、`retry_scheduled`、`timeout_triggered`、`concurrency_rejected`、`task_failed`、`task_succeeded`、`observability_write_failed`。
  ...
  - `RuntimeExecutionMetricSample` 至少必须包含：
    ...
    - `attempt_index`
    - `occurred_at`
  - `metric_name` 在 `v0.6.0` 的最小集合固定为：`task_started_total`、`task_succeeded_total`、`task_failed_total`、`attempt_started_total`、`retry_scheduled_total`、`timeout_total`、`concurrency_rejected_total`、`execution_duration_ms`。
  - 计数型指标的 `metric_value` 必须为非负整数；duration 型指标的 `metric_value` 必须为非负数且 `unit=ms`。
  - timeout / retry / concurrency 事实必须关联 `FR-0016` 的运行时结果，但不得在本 FR 中重新定义其调度、锁、队列或重试策略。
...
- 架构约束：
  ...
  - `FR-0016` 继续持有 timeout / retry / concurrency 运行时结果；本 FR 只规定这些结果如何出现在 failure observability 信号中。
...
Then `failure_phase` 可以是 `timeout`，并产生 `timeout_triggered` 日志与 `timeout_total` 指标，但 `error_category` 仍必须来自 `FR-0005` failed envelope

### 场景 4

Given 某次执行经历多次 retry 后耗尽重试预算  
When Core 收口最终失败  
Then 每次 retry 必须有可关联的 `attempt_index` 与 `retry_scheduled` 结构化日志，最终失败必须用同一 `task_id` 关联 failed envelope、TaskRecord 与 `retry_exhausted` 阶段

### 场景 5

Given 某次任务因 concurrency 限制被共享运行时拒绝进入执行  
When Core 产出 observability 信号  
Then 必须记录 `concurrency_rejected` 事件与 `concurrency_rejected_total` 指标；若该任务尚未进入 `accepted` TaskRecord 生命周期，则 `task_record_ref` 必须显式为 `none`
...
- 边界场景：
  - admission 前失败可以没有 TaskRecord，但必须显式表达 `task_record_ref=none`；进入 `accepted` 之后的失败必须关联同一 TaskRecord。
  - 资源 acquire 前失败可以没有 resource trace refs；资源 acquire 成功后发生的失败必须能关联已有 resource trace truth。
  - retry 的中间失败可以产生 attempt 日志和指标，但对外终态失败仍只能有一个最终 failed envelope。
...
- [ ] formal spec 明确 timeout / retry / concurrency 事实只作为 `failure_phase`、日志事件、指标或 runtime result 引用出现，不重写 `FR-0016`
...
- 外部依赖：
  ...
  - `FR-0016` 提供 timeout / retry / concurrency 运行时结果的上游语义，本 FR 只消费其结果引用
- 上下游影响：
