...
- contract 版本：`v0.6.0`
- 作用：定义 Core 在运行时失败发生后必须留下的最小结构化信号、日志与指标，以及它们与 task、TaskRecord、failed envelope、resource trace、timeout / retry / concurrency 结果的关联语义
...
- Runtime control result：来自 `FR-0016` 的 timeout / retry / concurrency 运行时结果
...
- `RuntimeFailureSignal`
  ...
  - 约束：`status` 固定为 `failed`；`error_category` 与 `error_code` 必须从 failed envelope 投影
- `RuntimeStructuredLogEvent`
  ...
  - 约束：失败相关日志必须引用对应 failure signal；生命周期日志不得把 `level` 当作错误分类
...
- 约束：`failure_phase` 不替代 `error_category`；timeout / retry / concurrency 不是新的顶层错误分类
...
- `concurrency_rejected_total`：单位 `count`
...
- Runtime control result：timeout / retry / concurrency 事实通过 `runtime_result_refs` 关联 `FR-0016` 结果，不在本 contract 中重新定义策略
