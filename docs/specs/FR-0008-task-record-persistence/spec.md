...
- 契约需求：
  ...
  - 对同一 `task_id` 的重复初始建档，只有在请求快照与既有 `accepted` 记录完全一致时才允许被视为 idempotent no-op；若请求快照不一致、或既有记录已进入更后状态，则必须视为冲突并 fail-closed。
  - 对同一 `task_id` 的重复终态写入，只有在终态状态、终态 envelope 与已落盘记录完全一致时才允许被视为 idempotent no-op；任何试图追加第二个不同终态、修改既有终态结果，或在终态后回退为非终态的操作都必须 fail-closed。
...
Given 同一 `task_id` 已经存在 durable `TaskRecord`
When 系统再次尝试为该 `task_id` 建档或再次写入终态
Then 只有与既有记录完全一致的重复写入才允许作为 idempotent no-op；任何请求快照不一致、第二个不同终态或终态回退都必须 fail-closed
...
- [ ] formal spec 明确冻结同一 `task_id` 下重复建档、重复终态写入与冲突写入的幂等/拒绝语义
