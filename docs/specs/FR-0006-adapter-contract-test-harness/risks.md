...
| contract violation 与合法失败 envelope 混淆 | 验证结论失真，后续实现无法准确归因，甚至私自发明新的运行时错误分类 | 验证工具输出至少区分通过、合法失败、contract violation、执行前置不满足；实现测试覆盖这四类最小分支，并保持运行时错误语义继承上位 contract | 回退结果分类改动，恢复上一版稳定判定逻辑 |
