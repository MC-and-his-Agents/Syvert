...
- 契约需求：
  ...
  - 平台泄漏检查的判定边界必须固定为：
    ...
    - 允许已批准的共享结果与错误 envelope 继续承载跨平台共用字段，例如 `normalized.platform` 作为共享最小字段、以及 `platform` 类失败在统一 `error.details` 中携带结构化平台细节；前提是这些字段不改变共享 envelope 的顶层语义与分类边界
    - 禁止平台专属、只服务单一平台或会导致共享层分叉的语义渗入 Core 主路径、共享输入模型、共享错误模型、adapter registry 共享契约、共享结果 contract（含 `raw` / `normalized` 的共享结果语义）、以及版本 gate 自身的共享判定逻辑
...
Then formal spec 必须要求两者都在共享 Core 契约下完成 `content_detail_by_url` 共享 operation 的最小回归矩阵，而不是只抽检单个平台、只跑任意单一路径或允许其中一个适配器跳过

### 场景 4

Given Core 主路径、共享模型或共享结果 contract 中出现平台名分支、平台专属 URL 规则、平台特定错误解释或平台专属 `normalized` 字段，且这些语义超出已批准的 `normalized.platform` 与统一 `error.details` 边界  
When 平台泄漏检查评估该改动  
