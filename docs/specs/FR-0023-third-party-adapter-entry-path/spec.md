...
- 契约需求：
  ...
  - 对第三方真实 `adapter_key` 的 resource declaration 准入必须同时满足两层覆盖：
    - declaration / profile 必须满足 `FR-0027` 的 shape、approved shared profile proof、tuple、execution path 与 fail-closed 规则。
    - 若 declaration `adapter_key` 不在被引用 `FR-0027` proof 的 `reference_adapters` 中，必须存在覆盖该 `adapter_key` 的 `ThirdPartyResourceProofAdmission`；否则按 `invalid_resource_requirement` fail-closed。
  - `ThirdPartyResourceProofAdmission` 只能把第三方 Adapter 的 fixture / manifest / contract profile 证据绑定到一个已批准的 `FR-0027` shared profile tuple；不得批准新共享能力词汇、不得把 `rejected` / `adapter_only` profile 升格为 shared、不得绕过 `FR-0027` proof coverage。
...
- 异常场景：
  ...
  - `ThirdPartyResourceProofAdmission` 绑定到 `rejected` / `adapter_only` profile、tuple 不一致、execution path 不一致、fixture / manifest 证据不可解析，或 admission `adapter_key` 与 manifest / declaration 不一致时，必须阻断准入。
