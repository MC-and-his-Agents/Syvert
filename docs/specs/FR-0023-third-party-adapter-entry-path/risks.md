...
| admission evidence identity 不可机器校验 | `#310` 只能靠约定或 reviewer 口头解释判断 evidence refs 是否绑定当前 manifest / fixture / profile | formal spec 冻结 `AdmissionEvidenceRef` schema，并要求 refs 可由当前 manifest、contract profile 与 fixtures 派生 | 回滚不可解析 refs，恢复当前 contract entry evidence identity |
