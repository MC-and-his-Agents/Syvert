...
| bootstrap 允许写入无 active lease 可解释的 `IN_USE` 资源 | snapshot 可能出现无法被 lease truth 解释的占用态，导致生命周期真相自相矛盾 | formal spec 明确：新建资源只允许 `AVAILABLE` / `INVALID`，`IN_USE` 仅允许作为既有 active truth 的 same-value replay；任何无 lease 可解释的 `IN_USE` seed 都必须 fail-closed | 回滚宽松 bootstrap 状态入口，恢复到“`IN_USE` 只能由 acquire + active lease 建立，active truth 仅可幂等重播”的单一路径 |
