...
Given `seed_resources(records)` 试图把此前不存在或与 durable truth 不一致的资源写成 `status=IN_USE`，但本次 bootstrap 输入并不允许同时建立 lease truth  
When Core 执行 bootstrap  
...
- 边界场景：
  ...
  - `seed_resources(records)` 不得成为把资源直接引入 `IN_USE` 的旁路；只有与既有 active truth 完全一致的 `IN_USE` 输入才允许作为 same-value replay，其余 `status=IN_USE` 输入都必须在 bootstrap 前置校验阶段被拒绝。
