...
- 功能需求：
  ...
  - `seed_resources(records)` 只允许注入 bootstrap-legal 的 `ResourceRecord` truth：由于 bootstrap surface 不得创建、删除或改写 lease truth，因此其输入记录只允许携带 `AVAILABLE` 或 `INVALID`；任一试图以 bootstrap 直接引入 `IN_USE` 资源的输入都必须在触达 durable truth 前 fail-closed，不得制造无 lease 可解释的占用态资源。
- 契约需求：
  ...
  - snapshot 中任一 `IN_USE` 资源都必须能够被当前 active `ResourceLease` 唯一解释；既然 `seed_resources(records)` 不得写入 lease truth，它也不得作为 `IN_USE` 资源进入 snapshot 的入口。
...
Given `seed_resources(records)` 试图写入 `status=IN_USE` 的 `ResourceRecord`，但本次 bootstrap 输入并不允许同时建立 lease truth  
When Core 执行 bootstrap  
Then Core 必须在触达 durable truth 前 fail-closed，不得把该资源写入 snapshot，也不得制造影子 `ResourceLease`
...
- 边界场景：
  ...
  - `seed_resources(records)` 不得成为把资源直接引入 `IN_USE` 的旁路；任何 `status=IN_USE` 的输入都必须在 bootstrap 前置校验阶段被拒绝。
...
- 本 FR 新增并冻结 `ResourceLifecycleSnapshot`、bootstrap surface 与默认本地 store-path 的 formal contract，但当前 PR 只补 formal traceability，不落盘运行时代码，也不执行任何仓内外数据迁移动作。
- 本次 formal traceability follow-up 不要求 schema 升级、默认路径迁移、历史本地 store 重写或数据回填；原因是主干上尚不存在已发布的 `FR-0010` durable snapshot/runtime rollout，本次只是在实现 PR `#176` 合入前把 canonical contract 收回 formal suite。
- 若后续实现 Work Item 首次创建本地 snapshot store，应按 `ResourceLifecycleSnapshot(schema_version=v0.4.0, revision=0, resources=[], leases=[])` 与 `SYVERT_RESOURCE_LIFECYCLE_STORE_FILE` / `~/.syvert/resource-lifecycle.json` baseline 建档；若未来需要修改 schema_version、路径策略或已有 durable truth 的迁移步骤，必须通过新的 formal spec 明确冻结。
