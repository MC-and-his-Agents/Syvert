...
- Core 必须在 `ResourceAdmissionDecision.evaluated_at` 使用 evidence 的 `observed_at`、`expires_at` 与 `freshness_policy_ref` 判定 freshness；过期 healthy evidence 必须投影为 `stale`。
- `ResourceHealthEvidence` 不得携带 `evaluated_at`，也不得使用 `status=unknown`；`unknown` 只属于 admission-time projection。
