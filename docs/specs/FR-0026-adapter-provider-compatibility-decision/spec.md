...
- 功能需求：
  ...
  - decision observability 只允许记录 decision id、adapter key、provider key、requirement id、offer id、capability、operation、matched profile keys、decision status、error code、contract refs 与 proof refs；provider key 只允许留在 Adapter-bound decision evidence，不得进入 Core-facing registry、TaskRecord、routing 或 resource lifecycle surface。
...
- 异常场景：
  ...
  - decision 试图把 provider key 或 provider selection 写入 Core routing、registry discovery、TaskRecord 或 resource lifecycle 时，必须返回 `invalid_contract` 或被 no-leakage guard 阻断。
- 边界场景：
  - 本 FR 允许 decision 在 Adapter-bound evidence 中记录 provider key、offer id 与 matched profile keys，但这些信息不得成为 Core-facing routing 或 persistence contract。
