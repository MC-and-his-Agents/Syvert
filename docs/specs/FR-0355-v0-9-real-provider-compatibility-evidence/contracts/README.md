...
- `adapter_bound_execution`：必须证明 matched decision 可以进入 Adapter-owned provider seam，并覆盖 provider 错误映射、resource profile consumption、resource lifecycle disposition hint 与 observability carrier。
...
- provider sample 不得只停留在 decision matrix；缺少 Adapter-bound execution evidence 时不得通过 `FR-0351` 的 `provider_compatibility_sample`。
