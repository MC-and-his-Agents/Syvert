...
| retry 被扩张为策略 DSL | v0.6.0 过早承担复杂调度、错误 predicate 与配置兼容负担 | spec 固定 `max_attempts`、固定 retryable outcome 与固定 `backoff_ms`，禁止自定义 predicate | revert DSL 相关实现，回到最小 RetryPolicy |
