...
- 本次纳入：
  ...
  - 冻结 `FR-0027` 可消费的 profile-level evidence truth 与 `ApprovedSharedResourceRequirementProfileEvidenceEntry` 投影
...
- 功能需求：
  ...
  - 自 `v0.8.0` 起，`FR-0027` 消费本 FR 时必须使用 profile-level evidence truth，而不是只读取 capability-level 词汇表后自行外推出合法 profile。
  - `ResourceRequirementProfileEvidenceRecord` 至少必须固定以下字段：
    - `profile_ref`
    - `capability`
    - `execution_path`
    ...
    - `decision`
    - `evidence_refs`
  - `profile_ref` 必须是非空、稳定、唯一字符串；它是 `FR-0027` declaration profile `evidence_refs` 可引用 proof 的来源。
  - `resource_dependency_mode` 只允许 `none` 或 `required`；当 mode=`none` 时 `required_capabilities` 必须为空，当 mode=`required` 时 `required_capabilities` 必须非空、去重。
  - profile-level `shared_status` 继续只允许 `shared`、`adapter_only`、`rejected`；profile-level `decision` 只允许：
    - `approve_profile_for_v0_8_0`
    - `keep_adapter_local`
    - `reject_profile_for_v0_8_0`
  - profile-level `shared_status` 与 `decision` 的组合必须满足：
    - `shared -> approve_profile_for_v0_8_0`
    - `adapter_only -> keep_adapter_local`
    - `rejected -> reject_profile_for_v0_8_0`
  - 只有 `shared + approve_profile_for_v0_8_0` 的 profile 才允许投影为 `ApprovedSharedResourceRequirementProfileEvidenceEntry`，并被 `FR-0027` shared declaration / matcher / adapter migration 消费。
  - 当前 `v0.8.0` 双参考 slice 只批准以下 shared profile：
    - `required + [account, proxy]`
    - `required + [account]`
  - 当前 `v0.8.0` 双参考 slice 明确不批准以下 profile 进入 shared declaration：
    - `required + [proxy]`：缺少两侧 reference adapter session 构造所需的 account material
    - `none`：缺少当前共享路径的双参考资源前提
    - 含 `verify_fp`、`ms_token`、`webid` 等 adapter 私有材料的 profile：只能保留为 `adapter_only`
- 契约需求：
  ...
  - `research.md` 必须显式保留三类内容：
    ...
    - 单平台特例
    - 被拒绝的抽象候选
    - `v0.8.0` profile evidence 判定口径与冻结 profile evidence truth
  - `research.md` 是本 FR 的证据补充入口，但不能成为第二套真相源；一切可批准能力仍以 `DualReferenceResourceCapabilityEvidenceRecord` 与本 spec 冻结的批准规则为准。
  - `FR-0027` 的 shared declaration 只能引用本 FR 投影出的 `ApprovedSharedResourceRequirementProfileEvidenceEntry`；不得引用 adapter 私有 profile、未批准候选或一次性运行日志。
...
Given `FR-0027` 需要判断 `content_detail_by_url + url + hybrid` 下的 `account+proxy` 与 `account-only` profile  
When 审查者读取 `FR-0015` 的 profile evidence truth  
Then 两个 profile 都必须以 `shared + approve_profile_for_v0_8_0` 投影为 `ApprovedSharedResourceRequirementProfileEvidenceEntry`

### 场景 7

Given `proxy-only` 或 `none` profile 进入 `FR-0027` shared declaration 讨论  
When evidence review 发现它们无法满足两侧 reference adapter 的 account material / session 前提  
Then 它们必须保持 `rejected + reject_profile_for_v0_8_0`，不得进入 shared declaration

### 场景 8

Given `verify_fp`、`ms_token`、`webid` 只在抖音 account material 中出现  
When 它们被作为 profile 维度的额外依赖观察  
Then 该 profile truth 只能保持 `adapter_only + keep_adapter_local`，不得投影为 `ApprovedSharedResourceRequirementProfileEvidenceEntry`
...
- 边界场景：
  ...
  - `FR-0015` 批准 `account-only` profile 不等于批准无账号、无资源或平台私有 token profile；这些 profile 必须继续由 profile evidence truth 显式拒绝或保留在 adapter 私有层。
  - `FR-0015` 对 `FR-0027` 只提供 profile approval proof，不定义 matcher 的 `one-of` 运行语义；matcher 语义仍由 `FR-0027` 持有。
...
- [ ] formal spec 明确冻结 `ResourceRequirementProfileEvidenceRecord` 与 `ApprovedSharedResourceRequirementProfileEvidenceEntry` 的字段集合与语义
- [ ] formal spec 明确冻结 `shared_status` 与 `decision` 的允许值和映射关系
- [ ] formal spec 明确冻结 `v0.5.0` 批准资源能力词汇仅为 `account`、`proxy`
- [ ] formal spec 明确冻结 `v0.8.0` 当前 shared profile 只批准 `account+proxy` 与 `account-only`
- [ ] formal spec 明确冻结 `proxy-only`、`none` 与 adapter 私有材料 profile 不得进入 shared declaration
