...
- 对 partial page，Core 固定消费 `result_status=partial_result` 与 `error_classification=parse_failed` 的组合语义；`partial_result` 继续保留为继承词表的兼容 entry，但不是本 FR 新增的 emitted error classification。
