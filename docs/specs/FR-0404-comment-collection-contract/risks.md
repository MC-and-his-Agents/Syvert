...
| placeholder 强制要求平台 source id | Adapter 会被迫伪造 unavailable/deleted/invisible comment identity，或把可保留的 placeholder 错误降级成 parse failure | spec 将 `source_id` 定义为可证明的平台稳定 id，placeholder 缺少该 id 时用可复验派生 `canonical_ref` 保留公共身份 | revert 强制字段，补充 placeholder identity fixture/evidence |
