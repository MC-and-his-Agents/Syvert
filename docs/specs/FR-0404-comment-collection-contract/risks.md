...
| root/parent/target linkage 语义不清 | Adapter 可能为不同平台投影出互相不兼容的 reply hierarchy，consumer 无法稳定消费 | spec 明确 `root_comment_ref`、`parent_comment_ref`、`target_comment_ref` 的最小含义与缺失边界 | revert 含糊字段定义，补充更严格的 model/test requirement |
