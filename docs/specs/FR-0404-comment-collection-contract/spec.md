...
- 功能需求：
  ...
  - `comment_list_by_content` 的后续请求输入固定为 `target + request_cursor` 组合：`request_cursor` 可以为空、可以设置 `page_continuation`，也可以设置某条 comment item 产出的 `reply_cursor`。
...
- 契约需求：
  ...
  - `comment_collection` 是 adapter-facing capability family；`comment_list_by_content` 是本 FR 冻结的 public executable operation。`FR-0368` 中的 proposed `comment_collection` registry entry 只能作为 reserved candidate，不得在 `#417/#418` 中继续作为 stable executable operation 名称。
...
Given `comment_list_by_content` 已返回第一页并生成 result `next_continuation`
...
Then item 必须返回 `visibility_status=unavailable` 与最小 normalized placeholder projection；若平台未提供稳定 comment id，`source_id` 必须使用 public placeholder namespace，并由 operation、target、visibility、stable placeholder marker 或 stable continuation/window slot key 派生，且不得依赖 raw payload ref、source trace 或 fetched timestamp
...
- [ ] formal spec 冻结 `comment_list_by_content` 的请求侧 cursor 规则，明确 result `next_continuation` 到 request `page_continuation` 的映射，以及 `page_continuation` 与 `reply_cursor` 的组合/互斥边界。
