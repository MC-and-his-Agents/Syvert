...
- 契约需求：
  ...
  - `source_id` 表示 Adapter 可证明的平台稳定 comment id；deleted/invisible/unavailable placeholder 缺少稳定平台 id 时，不得伪造 `source_id`，必须用可复验的派生 `canonical_ref` 保留公共 placeholder identity。
...
Then item 必须返回 `visibility_status=unavailable` 与最小 normalized placeholder projection；若平台未提供稳定 comment id，`source_id` 必须缺省，`canonical_ref` 必须由 target、raw payload ref、slot 与 source trace 派生，且不得默认降级成 collection-level `parse_failed`
