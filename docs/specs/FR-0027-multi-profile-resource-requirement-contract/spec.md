...
- 功能需求：
  ...
  - `FR-0027` 当前只冻结已批准 slice `content_detail_by_url + target_type=url + collection_mode=hybrid` 的 multi-profile contract。没有新增 execution-path 证据前，本事项下的 declaration、proof 与 matcher 都不得把 approved profile 复用到其它 `content_detail` 路径。
  ...
  - declaration profile 上的 `evidence_refs` 在当前 `v0.8.0` 必须是长度恰为 1 的字符串数组；这唯一一个引用必须精确且唯一地命中一个 `ApprovedSharedResourceRequirementProfileEvidenceEntry.profile_ref`，并且该 entry 的 `capability`、`execution_path`、`resource_dependency_mode`、`required_capabilities` 必须与 declaration 的当前 approved slice和该 profile tuple 完全一致，同时其 `reference_adapters` 必须覆盖 declaration 的 `adapter_key`；不得引用 adapter 私有注释、运行期临时日志或未批准材料。
...
- 契约需求：
  - 以下情况必须归类为 `runtime_contract`，并固定 `error.code=invalid_resource_requirement`：
    ...
    - `evidence_refs` 为空、重复、长度不等于 1，或无法解析到 `ApprovedSharedResourceRequirementProfileEvidenceEntry.profile_ref`
    - 任一 `evidence_ref` 命中多个 approval proof，或 approval proof 的 `profile_ref` 在 carrier 中不唯一
    - declaration 中包含无法与 `ApprovedSharedResourceRequirementProfileEvidenceEntry` 在 `capability + execution_path + resource_dependency_mode + required_capabilities` 上完全对齐的 profile
...
Given 某条 declaration 中存在 `required + [account]` profile，但该 profile 的 `evidence_refs` 无法唯一回指与 declaration `capability`、当前 approved execution path 和该 profile tuple 完全对齐的 `ApprovedSharedResourceRequirementProfileEvidenceEntry`  
...
- 边界场景：
  ...
  - `FR-0027` 当前不把 approved profile 从 `content_detail_by_url + url + hybrid` 自动外推到其它 `content_detail` 路径；若后续需要新增执行路径，必须先补新的 evidence 与新的 formal spec follow-up。
...
- [ ] formal spec 明确冻结当前 approved execution path 与 declaration-profile 单 proof 绑定规则
