...
| 未经 `FR-0015` 批准的 profile 被直接塞进 declaration | shared contract 被 `adapter_only` 宽松路径污染，后续开放接入 baseline 不可信 | declaration 强制绑定 profile-level evidence refs，未批准 profile 一律 fail-closed | 回滚违规 profile，并回到 evidence carrier 重新审查 |
