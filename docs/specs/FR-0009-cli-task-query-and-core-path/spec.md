...
- 功能需求：
  ...
  - `run --capability` 的 public 值域在 `v0.3.0` 固定为调用侧 operation id `content_detail_by_url`；formal spec 不允许把 public CLI 值域改写成 adapter-facing capability family。
  - `run` 语义必须承接该固定请求载体，并把它投影到 `FR-0004` / `FR-0008` 已冻结的共享请求模型，再沿同一条 Core 执行路径完成 admission、adapter 执行、`TaskRecord` durable 写入与终态输出。
  - 对 `run --capability=content_detail_by_url` 的内部共享语义，formal spec 固定采用 `FR-0004` 已冻结的兼容投影：调用侧 operation id `content_detail_by_url` 在进入共享请求模型后投影到 adapter-facing capability family `content_detail`，并与 `target_type=url`、`target_value=<url>`、`collection_mode=hybrid` 一起组成共享请求语义。
  - legacy 平铺执行入口 `python -m syvert.cli --adapter ... --capability ... --url ...` 必须继续兼容，并映射到与 `run` 完全等价的请求载体与执行语义。
  ...
  - `run` 与 legacy 平铺入口的参数形状错误同样属于本 FR 的 public CLI contract：缺少必填参数、出现未知参数或子命令参数形状不合法时，CLI 必须返回 shared failed envelope，且固定 `error.code=invalid_cli_arguments`、`error.category=invalid_input`。
  - 对 `run` 与 legacy 平铺入口的参数形状错误：
    - `task_id` 必须使用新生成的非空 fallback CLI `task_id`
    - 若 malformed argv 中可恢复 `--adapter <adapter_key>` 或 `--capability <capability>`，则 failed envelope 必须回显这些值；不可恢复时对应字段固定为空字符串
    - 若 fallback CLI `task_id` 的生成或校验本身失败，则必须优先返回共享 `invalid_task_id` failed envelope；此时 `invalid_task_id` / `runtime_contract` 优先于 `invalid_cli_arguments` / `invalid_input`
...
- 契约需求：
  ...
  - `query` 缺少 `--task-id`、携带未知参数或子命令参数形状不合法时：
    ...
    - 只有在 `--task-id` 缺失、值缺失或无法从 malformed argv 中恢复查询键时，`task_id` 才允许退回到新生成的非空 fallback CLI `task_id`
    - `adapter_key=""`
    - `capability=""`
    - 若上述 fallback CLI `task_id` 的生成或校验本身失败，则必须优先返回共享 `invalid_task_id` failed envelope；此时 `invalid_task_id` / `runtime_contract` 优先于 `invalid_cli_arguments` / `invalid_input`
  ...
  - store 根目录不可用、invalid marker、JSON 损坏、contract 非法、记录不可读、权限或 IO 异常时：
    ...
    - `adapter_key=""`
    - `capability=""`
  - 一旦 durable record 已成功加载，后续若在共享序列化或 CLI 输出阶段仍需构造 failed envelope：
    - `error.code` 固定为 `task_record_unavailable`
    - `error.category` 固定为 `runtime_contract`
    - `task_id` 固定回显该 durable record 的 `task_id`
    - `adapter_key` 与 `capability` 必须回填自 `record.request`，不得继续留空
    - 该分支表示“record 可读但无法被安全输出”，不改变 `task_record_unavailable` 对 pre-load / load-failure 分支的 fail-closed 语义
...
Given 用户使用 `run --capability content_detail_by_url`  
When CLI 把该请求投影到共享请求模型  
Then public CLI 仍固定暴露 `content_detail_by_url`，而内部只允许按 `FR-0004` 兼容规则投影到 adapter-facing capability family `content_detail`

### 场景 1B

Given 用户通过 `run` 或 legacy 平铺入口提交的 CLI 参数形状不合法  
When CLI 在进入 durable `accepted` 生命周期前构造失败输出  
Then 它必须返回 `invalid_cli_arguments` / `invalid_input` 的 shared failed envelope，并在无法恢复持久化任务键时使用新生成的非空 fallback CLI `task_id`

### 场景 1C

Given 用户通过 `run`、legacy 平铺入口或 `query` 提交的 CLI 参数形状不合法，且 fallback CLI `task_id` 生成或校验失败  
When CLI 构造失败输出  
Then 它必须优先返回共享 `invalid_task_id` / `runtime_contract` failed envelope，而不是继续输出 `invalid_cli_arguments`
...
When CLI 构造失败输出  
Then 它必须返回 `invalid_cli_arguments` failed envelope；若 malformed argv 中仍可恢复 `--task-id` 则必须回显该值，否则继续使用新生成的非空 fallback CLI `task_id`
...
Given `query` 已成功加载某条 durable `TaskRecord`  
When 共享序列化或 CLI 输出阶段失败  
Then CLI 必须返回 `task_record_unavailable` / `runtime_contract` failed envelope，并回填该 record 的 `task_id`、`adapter_key` 与 `capability`
...
- 异常场景：
  ...
  - 若 subcommand 改造导致 `run` 或 legacy 平铺入口的参数错误不再返回 `invalid_cli_arguments` / `invalid_input` shared failed envelope，或不再保留对 `adapter_key` / `capability` 的可恢复回显，则构成回归。
- 边界场景：
