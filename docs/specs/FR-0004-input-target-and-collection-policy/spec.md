...
- 契约需求：
  ...
  - 对 `FR-0002` 兼容投影得到的 `collection_mode=hybrid`，Core admission 必须采用兼容特例：若 adapter 仅声明 `public`，或仅声明 `authenticated`，仍视为可合法承接该 legacy 请求；只有 native `FR-0004` 调用方显式提交 `collection_mode=hybrid` 时，Core 才按 adapter 是否显式声明 `hybrid` 来执行常规 admission 规则。
