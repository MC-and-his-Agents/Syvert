...
Given 调用方提交 `adapter_key=xhs`、`capability=content_detail`、`target_type=url`、`target_value=<xhs-detail-url>`，且 `collection_mode=authenticated`  
...
Then 该既有 URL 输入必须能无损映射为 `target_type=url` 与 `target_value=input.url`，而不改变单目标 detail 语义
