...
- 关联 PR：`N/A（requirement container 不直接承载 PR；当前 formal spec 执行 PR 见 #241）`
...
- 当前执行回合的 formal spec 语义基线绑定到 `2234fc8e20d53aeb4f1d436d662a9f5d44d7b4c1`；该 checkpoint 已冻结最小 HTTP transport contract、canonical endpoint path / method、固定状态码映射与 shared failed envelope transport carrier。其后若只追加 PR / checks / review-sync metadata，不改写本 requirement container 的共享语义。
