...
- Runtime execution now maps `comment_collection + paginated` to managed `account/proxy` resource slots before adapter execution, matching the declared consumer/resource contract.
...
- `python3 -m unittest tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_builds_comment_collection_success_envelope_from_adapter_payload tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_fails_closed_when_comment_collection_resource_pool_is_empty tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_passes_comment_request_cursor_to_adapter_context tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_normalizes_dataclass_cursor_before_adapter_context`（4 tests）
