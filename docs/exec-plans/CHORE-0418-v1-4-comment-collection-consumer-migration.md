...
- `python3 -m unittest tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_uses_v1_comment_collection_account_only_declaration tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_uses_comment_collection_account_only_profile tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_builds_comment_collection_success_envelope_from_adapter_payload tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_returns_resource_unavailable_when_runtime_capability_projection_is_unmatched tests.runtime.test_runtime.RuntimeExecutionTests.test_execute_task_maps_v2_unmatched_profiles_to_resource_unavailable`（5 tests）
...
- PR #430 guardian finding：V1 `comment_collection` declarations with `required_capabilities=("account",)` still defaulted to the operation-wide `account+proxy` slot set.
- 处理结果：derived runtime slots from V1 `comment_collection` declarations as well as V2 profiles, added a V1 account-only adapter/resource-pool success regression, and reran the full #418 regression subset.
