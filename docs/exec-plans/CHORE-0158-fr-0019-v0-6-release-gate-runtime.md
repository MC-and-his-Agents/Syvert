...
- `env -u GH_TOKEN -u GITHUB_TOKEN python3 scripts/pr_guardian.py review 252 --post-review --json-output /tmp/syvert-guardian-252-37ab71e.json`
  - 结果：`REQUEST_CHANGES`；指出 renderer 虽运行上游测试，但 gate payload 仍直接消费静态 source artifact 的 actual_result / metrics / policy；同时 local case ref 未打开 artifact 校验内容与 payload 绑定。
- `env -u GH_TOKEN -u GITHUB_TOKEN python3 scripts/pr_guardian.py review 252 --post-review --json-output /tmp/syvert-guardian-252-c2b7908.json`
  - 结果：`REQUEST_CHANGES`；指出 renderer 重写后未消费 source case-level refs、runtime artifact 仍可能自证 actual_result，以及 source validation 将所有 extension case 拒绝。
- `docs/exec-plans/artifacts/CHORE-0158-operability-source-evidence.json`
