...
- guardian 对旧 head 的审查先后指出了三类阻断：formal spec 回退了上游共享请求 contract / 泄漏实现细节，`invalid_cli_arguments` 在可恢复 `--task-id` 时仍可能丢失用户查询键，以及 `plan.md` 中存在与串行收口策略不一致的并行描述。
- 当前分支已把 `FR-0009` spec 收敛回 `FR-0004` / `FR-0008` 已冻结的共享请求模型与 durable-truth contract，补齐“可恢复 query key 时必须回显用户 `task_id`”的参数错误 contract，并把 legacy URL 兼容投影与 `#141 -> #142 -> #143 -> #144` 串行关系写死；当前停点是等待绑定最新 head 的 guardian 复核。
...
- guardian 旧 head 审查摘要
  - 结果：指出 shared request contract drift、实现细节泄漏到 formal spec、`invalid_cli_arguments` 可能丢失可恢复的查询键，以及 `plan.md` 存在并行描述；当前分支已据此修正，待最新 head 复核
...
- `fabaf73fdd17f0eb42f1f1f2ee97bf0d96e08772`
