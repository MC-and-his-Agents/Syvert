...
| legacy run 与 `run` 子命令必须写入等价 durable truth | `#143` | legacy run path + `run` path -> shared durable record | `n/a` | `0` | `n/a` | `test_run_subcommand_and_legacy_entrypoint_persist_equivalent_durable_truth` | `implemented` |
| `query` 不得消费 shadow file / shadow payload | `#143` | `execute_query_command()` -> `default_task_record_store().load()` -> `task_record_to_dict()`，且额外文件系统咨询必须 fail-closed | `stdout=task_record_to_dict(record)` / `stderr=""` | `0` | `n/a` | `test_query_subcommand_reads_loaded_record_via_shared_store_and_shared_serializer` | `implemented` |
