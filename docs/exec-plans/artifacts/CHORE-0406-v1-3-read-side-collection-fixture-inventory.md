...
| `target_not_found_collection` | `content_list_by_creator` | `synthetic` | creator target is absent, unavailable, or not resolvable on the platform | collection result returns `error_classification=target_not_found` and does not masquerade as `empty_result` | `missing` |
