...
- “real adapter regression” 的公开语义固定为：真实 `XhsAdapter` / `DouyinAdapter` 类实例经 `syvert.runtime.execute_task()` 进入公开 runtime 边界执行，允许使用 transport seam 注入 hermetic fixture；这不是 fake/stub adapter，也不是 live network 成功证明。
- 允许的 operation surface 只包括：
  - `operation=content_detail_by_url`，并归一化到 `target_type=url`
  - `operation=content_detail` 且 `target_type=url`
- `adapter_results[*].cases[*]` 的 `case_id` / `expected_outcome` / `evidence_ref` 必须与版本冻结矩阵逐 adapter 精确一致，不能只满足粗粒度 coverage 或顶层 `evidence_refs` 顺序
- `adapter_results[*].cases[*]` 必须显式携带非空 `evidence_ref`
- 每个 adapter 至少一条 success case
- 每个 adapter 至少一条允许失败 case
- 允许失败类别只允许 `invalid_input` / `platform`
- 顶层 `evidence_refs` 必须与冻结矩阵逐 case `evidence_ref` 按 adapter/case 顺序完全一致，缺失、错绑或乱序都必须 fail-closed
- 即使 reference adapter 绑定在 builder 阶段就已 fail-closed，`run_real_adapter_regression()` 产出的顶层 `evidence_refs` 也必须继续保持冻结矩阵 canonical order，binding-specific trace 只能进入 failure details，不得覆盖顶层证据绑定。
...
对 `v0.2.0`，`boundary_scope` 必须按冻结顺序精确匹配：
...
同一集合但顺序颠倒，必须按 fail-closed 处理，而不是作为等价 producer 输出继续放行。
