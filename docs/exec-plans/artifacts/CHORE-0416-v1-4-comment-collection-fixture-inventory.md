...
| `unavailable_comment_item` | `comment_collection` | `synthetic` | one comment placeholder is returned with stable unavailable marker but without full body payload or stable platform comment id | item visibility is `unavailable`; `source_id` stays absent when unprovable, while `canonical_ref` is derived from target/raw-ref/slot/source trace | `synthetic_derivable` |
