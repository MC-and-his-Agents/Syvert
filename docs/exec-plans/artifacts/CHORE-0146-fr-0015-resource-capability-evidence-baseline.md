# CHORE-0146 FR-0015 Resource Capability Evidence Baseline

本文件把 `#211 / CHORE-0146` 已落地的 `FR-0015` machine-readable evidence baseline 摘出来，供 code review、closeout 与后续 `#195/#196` 直接消费。
...
| `fr-0015:xhs:content-detail:url:hybrid:url-request-tokens` | `syvert/adapters/xhs.py` | `build_detail_body` | `xhs` adapter 只在平台私有 detail body 中透传 `xsec_token` 与 `xsec_source` 两个 URL / request token |
| `fr-0015:douyin:content-detail:url:hybrid:request-signature-token` | `syvert/adapters/douyin.py` | `DouyinAdapter._build_detail_params` | `douyin` adapter 在平台私有签名步骤中生成并注入 `a_bogus` 请求 token |
| `fr-0015:xhs:content-detail:url:hybrid:page-state-fallback` | `syvert/adapters/xhs.py` | `XhsAdapter._recover_note_card_from_html` | `xhs` adapter 在 detail / html 路径失败时会退回 browser page-state 恢复链路，这属于技术绑定回退 |
| `fr-0015:douyin:content-detail:url:hybrid:page-state-fallback` | `syvert/adapters/douyin.py` | `DouyinAdapter._recover_aweme_detail_from_page_state` | `douyin` adapter 在 detail 路径失败时会退回 browser page-state 恢复链路，这属于技术绑定回退 |
...
| `douyin` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `adapter_private_request_token=a_bogus` | `a_bogus` | `rejected` | `reject_for_v0_5_0` | `fr-0015:douyin:content-detail:url:hybrid:request-signature-token` |
| `xhs` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `adapter_private_request_token=xsec_token` | `xsec_token` | `rejected` | `reject_for_v0_5_0` | `fr-0015:xhs:content-detail:url:hybrid:url-request-tokens` |
| `xhs` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `adapter_private_request_token=xsec_source` | `xsec_source` | `rejected` | `reject_for_v0_5_0` | `fr-0015:xhs:content-detail:url:hybrid:url-request-tokens` |
...
| `xhs` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `account_material_field=cookies` | `cookies` | `rejected` | `reject_for_v0_5_0` | `fr-0015:xhs:content-detail:url:hybrid:account-material` |
| `douyin` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `account_material_field=cookies` | `cookies` | `rejected` | `reject_for_v0_5_0` | `fr-0015:douyin:content-detail:url:hybrid:account-material` |
| `xhs` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `account_material_field=user_agent` | `user_agent` | `rejected` | `reject_for_v0_5_0` | `fr-0015:xhs:content-detail:url:hybrid:account-material` |
| `douyin` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `account_material_field=user_agent` | `user_agent` | `rejected` | `reject_for_v0_5_0` | `fr-0015:douyin:content-detail:url:hybrid:account-material` |
| `xhs` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `technical_binding_candidate=browser_state` | `browser_state` | `rejected` | `reject_for_v0_5_0` | `fr-0015:xhs:content-detail:url:hybrid:page-state-fallback` |
| `douyin` | `content_detail` | `operation=content_detail_by_url, target_type=url, collection_mode=hybrid` | `technical_binding_candidate=browser_state` | `browser_state` | `rejected` | `reject_for_v0_5_0` | `fr-0015:douyin:content-detail:url:hybrid:page-state-fallback` |
