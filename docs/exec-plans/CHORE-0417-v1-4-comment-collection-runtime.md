...
- `python3 -m unittest tests.runtime.test_models tests.runtime.test_comment_collection tests.runtime.test_read_side_collection tests.runtime.test_operation_taxonomy tests.runtime.test_operation_taxonomy_admission_evidence tests.runtime.test_runtime tests.runtime.test_task_record tests.runtime.test_registry tests.runtime.test_resource_capability_evidence tests.runtime.test_platform_leakage tests.runtime.test_real_adapter_regression tests.runtime.test_cli_http_same_path tests.runtime.test_version_gate tests.runtime.test_third_party_adapter_contract_entry`（510 tests）
- `python3 -m unittest tests.runtime.test_comment_collection.CommentCollectionCarrierTests.test_complete_success_page_with_comment_items_is_valid tests.runtime.test_comment_collection.CommentCollectionCarrierTests.test_collection_level_failure_with_items_is_rejected tests.runtime.test_comment_collection.CommentCollectionCarrierTests.test_reply_window_allows_descendant_reply_within_non_root_resumed_thread tests.runtime.test_runtime.RuntimeExecutionTests.test_validate_success_payload_accepts_non_empty_comment_complete_page tests.runtime.test_runtime.RuntimeExecutionTests.test_validate_success_payload_accepts_non_root_descendant_reply_window`
...
- PR `#429` guardian finding：adapter 可通过修改可变 `request_cursor` 绕过 reply-thread drift 校验。
  - 处理：runtime 现在在 adapter 执行前分别 deep-copy adapter 可见 cursor 与 validation cursor snapshot，成功态校验只使用 validation snapshot；contract harness 同样隔离 adapter 可见 cursor 与 sample/build validation cursor，并补 adapter cursor mutation regression。
