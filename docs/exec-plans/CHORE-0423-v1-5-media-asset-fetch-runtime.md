...
- PR guardian 第八轮 findings 已处理：request/target/lineage media ref 均做值级脱敏校验；后续系统核对确认 `no_storage` 不是 FR-0405 public envelope 字段，因此改为未规约字段 fail-closed。
...
- 系统性核对补正：`allowed_content_types` policy priority 现在覆盖所有非 complete carrier，downloaded-bytes audit proof 必须与 public metadata 下载事实一致。
...
- 系统性核对补正：`origin_ref`/`policy_ref` 作为可选 target public refs 在 runtime 与 TaskRecord 均允许且脱敏校验；`no_storage` 不作为 public carrier 持久化，runtime/TaskRecord 对该未规约字段统一 fail-closed。

## FR-0405 media carrier 不变量核对

- public carrier 字段必须白名单：`target`、`fetch_policy`、`media`、`media.metadata`、`media.source_ref_lineage`、`source_trace`、`audit` 不接受未定义扩展字段；`no_storage` 不属于 FR-0405 public envelope，出现时 fail-closed。
...
- privacy/no-storage 边界固定：source trace、lineage、metadata、audit 均拒绝 URL、签名参数、session/credential、provider routing/fallback/selector、bucket/storage/download/local path；不得通过未规约 `no_storage` 字段表达存储状态。
