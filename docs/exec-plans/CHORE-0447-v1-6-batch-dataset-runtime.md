...
- PR `#452`：open；latest remote head `671b9872158ac46a37c15f412abbf870419a8d10` GitHub checks 全绿，guardian `APPROVE`，merge gate `REQUEST_CHANGES`。Merge gate blocker：search/list batch `request_cursor.continuation_token` 与 read-side `next_continuation.continuation_token` 可携带 private path / storage handle / token。已在 runtime remediation commit `c5af19eb544edbc389e49341a0b0bcaba130c9d7` 修复：request-side continuation token 进入 typed batch wrapper 前执行 public-safe 校验；result-side `next_continuation` cursor token 在 `BatchItemOutcome` public validation 中递归校验。下一步是推送 evidence sync，等待 GitHub checks，然后只运行一次 guardian，再只运行一次 merge gate。
