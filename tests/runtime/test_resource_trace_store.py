...
class ResourceTraceStoreTests(ResourceTraceStoreEnvMixin, unittest.TestCase):
    ...
    def drifted_resource_type_released_event(self) -> ResourceTraceEvent:
        return ResourceTraceEvent(
            **{**self.released_event().__dict__, "resource_type": "proxy"}
        )
    ...
    def alternate_adapter_proxy_acquired_event(self) -> ResourceTraceEvent:
        return ResourceTraceEvent(
            **{**self.proxy_acquired_event().__dict__, "adapter_key": "douyin"}
        )

    def alternate_capability_proxy_acquired_event(self) -> ResourceTraceEvent:
        return ResourceTraceEvent(
            **{**self.proxy_acquired_event().__dict__, "capability": "author_profile"}
        )
    ...
    def test_append_events_rejects_resource_type_drift_within_same_resource_timeline(self) -> None:
        store = self.make_store()
        store.append_events((self.acquired_event(),))

        with self.assertRaises(ResourceTracePersistenceError):
            store.append_events((self.drifted_resource_type_released_event(),))

    def test_append_events_rejects_adapter_or_capability_drift_within_same_lease(self) -> None:
        store = self.make_store()
        store.append_events((self.acquired_event(),))

        with self.assertRaises(ResourceTracePersistenceError):
            store.append_events((self.alternate_adapter_proxy_acquired_event(),))
        with self.assertRaises(ResourceTracePersistenceError):
            store.append_events((self.alternate_capability_proxy_acquired_event(),))
    ...
    def test_load_events_rejects_resource_type_or_adapter_drift(self) -> None:
        store = self.make_store()
        store.path.write_text(
            "\n".join(
                (
                    '{"adapter_key":"xhs","bundle_id":"bundle-001","capability":"content_detail_by_url","event_id":"acquired:lease-001:account-001","event_type":"acquired","from_status":"AVAILABLE","lease_id":"lease-001","occurred_at":"2026-04-21T12:00:00.000000Z","reason":"acquired_for_task","resource_id":"account-001","resource_type":"account","task_id":"task-001","to_status":"IN_USE"}',
                    '{"adapter_key":"douyin","bundle_id":"bundle-001","capability":"content_detail_by_url","event_id":"acquired:lease-001:proxy-001","event_type":"acquired","from_status":"AVAILABLE","lease_id":"lease-001","occurred_at":"2026-04-21T12:00:00.000000Z","reason":"acquired_for_task","resource_id":"proxy-001","resource_type":"proxy","task_id":"task-001","to_status":"IN_USE"}',
                    "",
                )
            ),
            encoding="utf-8",
    ...
    def test_projection_builders_reject_resource_type_or_adapter_or_capability_drift(self) -> None:
        with self.assertRaises(ResourceTraceContractError):
            build_task_resource_usage_log(
                (self.acquired_event(), self.drifted_resource_type_released_event()),
                task_id="task-001",
            )
        with self.assertRaises(ResourceTraceContractError):
            build_resource_lease_timeline(
                (self.acquired_event(), self.alternate_adapter_proxy_acquired_event()),
                lease_id="lease-001",
            )
        with self.assertRaises(ResourceTraceContractError):
            build_resource_lease_timeline(
                (self.acquired_event(), self.alternate_capability_proxy_acquired_event()),
                bundle_id="bundle-001",
            )
